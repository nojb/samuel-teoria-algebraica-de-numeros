\documentclass[10pt,oneside,bibtotoc,smallheadings,leqno,a5paper,DIV=12]{scrbook}

%,numbers=noenddot
%paper=8.4in:5.9in
%paper=a5paper

\usepackage[colorlinks,linkcolor=blue,linktocpage=false,pagebackref=true]{hyperref}
...
\newenvironment{comm}%
%	{\begin{labeling}{Exam}\item{}\small}
%	{\end{labeling}}	
	{\begin{trivlist}\item\small\itshape}
	{\end{trivlist}}

\newtheoremstyle{defi}{\topsep}{\topsep}{}{}{\bfseries}{.}{.5em}{}
...
\newtheoremstyle{enonce}{\topsep}{\topsep}{\slshape}{}{\bfseries}{.}{.5em}{}
...
\newtheoremstyle{rem}{\topsep}{\topsep}{}{}{\slshape}{.}{.5em}{}
...
\addchap{Repaso de notaciones, definiciones y resultados}
...
f\'ormulas~\eqref{eq-1.2-2} se deducen multiplicando de nuevo el m.c.d. por $d$. %\QED
\end{proof}
...
que muestra que $q$ es coprimo con $n$.%\QED
\end{proof}
...
{\em conjugado} en $K$, concretamente $-\sqrt{d}$. Por lo tanto existe un automorfismo $\sigma$
...
Nos proponemos estudiar ahora el anillo $A$ de {\em enteros} de $K$, es decir, el conjunto de
...
Para terminar, un poco de {\em terminolog\'ia.} Si $d > 0$, decimos que $\QQ(\sqrt{d})$ es un {\em cuerpo
cuadr\'atico real} (pues existe un subcuerpo de $\RR$ conjugado a $\QQ(\sqrt{d})$ sobre $\QQ$). Si $d < 0$,
decimos que $\QQ(\sqrt{d})$ es un {\em cuerpo cuadr\'atico imaginario.}

\section{Norma y traza}\label{sec2.6}

\subsection*{a) Repaso de \'algebra lineal}

Sea $A$ un anillo, $E$ un $A$-m\'odulo {\em libre} de rango finito y $u$ un endomorfismo de $E$.
En \'algebra lineal se define la {\em traza,} el {\em determinante} y el {\em polinomio caracter\'istico}
...
En particular, apliquemos~\eqref{eq2.7.6} al caso donde $F(X)$ es un {\em trinomio}
...
Llamamos {\em cuerpo de n\'umeros algebraicos} (o {\em cuerpo de n\'umeros}) a toda extensi\'on de
$\QQ$ de grado finito (y por lo tanto algebraica). Dado un cuerpo de n\'umeros $K$, el grado
$[K:\QQ]$ se llama {\em grado} de $K$. Un cuerpo de n\'umeros de grado $2$ (resp. $3$) se llama
{\em cuerpo cuadr\'atico} (cf.~\S\ref{sec2.5}) (resp. {\em cuerpo c\'ubico}). Un cuerpo de n\'umeros
tiene caracter\'istica cero.

Dado un cuerpo de n\'umeros $K$, los elementos de $K$ que son enteros sobre $\ZZ$ se llaman
{\em enteros} de $K$. Forman un {\em subanillo} $A$ de $K$ (\S\ref{sec2.1}, corolario~\ref{cor2.1.2}
de la proposici\'on~\ref{prop2.1.1}) que es un
$\ZZ$-m\'odulo {\em libre} de rango $[K:\QQ]$ (\S\ref{sec2.7}, corolario del teorema~\ref{teo2.7.1}). Los discriminantes de
...
que los discriminantes de dos bases cualesquiera del $\ZZ$-m\'odulo $A$ son {\em iguales;} su valor
en com\'un se llama {\em discriminante absoluto,} o {\em discriminante,} de $K$.
...
\addsec{Ap\'endice. El cuerpo de los n\'umeros complejos es algebraicamente cerrado}%
...
Sea ahora $A$ un {\em dominio \'integro} y $K$ su cuerpo de fracciones. Llamamos {\em ideal fraccionario} de $A$
...
Los ideales ordinarios de $A$ son ideales fraccionarios (con $d=1$). Los llamamos {\em ideales enteros} si
hay riezgo de confusi\'on. Todo sub-$A$-m\'odulo {\em de tipo finito} $I$ de $K$ es un ideal fraccionario.
...
$a_{i},d_{i}\in A$) y $d$ sirve de denominador com\'un de $I$. Rec\'iprocamente, si $A$ es {\em noetheriano,}
todo ideal fraccionario $I$ es un $A$-m\'odulo de {\em tipo finito:} en efecto, se tiene $I\subset d^{-1}A$ y
$d^{-1}A$ es un $A$-m\'odulo isomorfo a $A$, luego noetheriano.

Definimos el {\em producto} $II'$ de dos ideales fraccionario $I$, $I'$ como el conjunto de sumas finitas
...
\end{gather*}
son {\em ideales fraccionarios.} En efecto, es claro que son sub-$A$-m\'odulos de $K$ y admiten como denominador
com\'un, respectivamente, a $d$ (o $d'$), $dd'$ y $dd'$. Los ideales fraccionarios no nulos de $A$ forman un
{\em monoide} conmutativo con la multiplicaci\'on.
...
(\S\ref{sec4.4}, teorema~\ref{teo4.4.1}). Estas forman un grupo {\em finito c\'iclico.} Redemostraremos este resultado con un sencillo
...
Este grupo admite un s\'olo generador $>1$. Lo llamamos {\em la unidad fundamental de $K$.}
...
es $>1$ y es el m\'as grande de los cuatro. Por lo tanto {\em las unidades $>1$ de $K$ son las unidades de la
...
(llamada {\em ``ecuaci\'on de Pell-Fermat''}\/) se obtienen de la siguiente manera: tomamos la unidad fundamental
$a_{1}+b_{1}\sqrt{d}$ de $K$ y ponemos
\begin{gather}\label{eq-4.6-2}
a_{n}+b_{n}\sqrt{d} = (a_{1}+b_{1}\sqrt{d})^{n}\quad(n\geq 1).
\end{gather}
La sucesi\'on $(a_{n},b_{n})$ provee ???? entonces {\em todas las soluciones} de~\eqref{eq-4.6-1}.

\begin{remark*}[Observaciones]
1) Resulta de~\eqref{eq-4.6-2} que $b_{n+1}=a_{1}b_{n}+b_{1}a_{n}$. Como $a_{1}, b_{1}, a_{n}, b_{n}>0$,
la sucesi\'on $(b_{n})$ es estrictamente creciente. As\'i, para {\em calcular} expl\'icitamente la unidad
...
\addsec{Ap\'endice. Un c\'alculo de vol\'umen\protect\sectionmark{Un c\'alculo de vol\'umen}}
...
Sea $q$ un n\'umero primo {\em impar} y $K$ el cuerpo ciclot\'omico generado por una ra\'iz primitiva
$q$-\'esima de la unidad en $\CC$. El grupo de Galois $G$ de $K$ sobre $\QQ$ es isomorfo a $\FF_{q}^{*}$
(\S\ref{sec6.4}, teorema~\ref{teo6.4.1}, {\itshape b})) y por lo tanto es {\em c\'iclico} de orden par $q-1$. Por lo tanto admite
un \'unico subgrupo $H$ de \'indice $2$, que corresponde al subgrupo de los cuadrados $(\FF_{q}^{*})^{2}$.
As\'i $K$ contiene un \'unico subcuerpo {\em cuadr\'atico} $F$ (\S\ref{sec6.1}, teorema~\ref{teo6.1.2}, {\itshape b})). Ning\'un n\'umero
...
$=$ a la clase de $p\bmod q$ (cf.~\S\ref{sec6.4}) es un {\em cuadrado} en $\FF_{q}^{*}$. En el caso contrario el el
...
\addchap{Complementos sin demostraci\'on}
...
\addsec*{F\'ormulas de transitividad}
...
\addsec*{Norma relativa de un ideal}
...
$\idl{p}'$ de $A'$ {\em es ramificado} sobre $A$ (o sobre $K$) si su \'indice de ramificaci\'on sobre $A$ es $>1$.
Entonces el ideal maximal $\idl{p} = \idl{p}'\cap A$ de $A$ {\em ramifica} en $A'$ (cap\'itulo~\ref{cap5}, \S\ref{sec5.3}).
Resulta f\'acilmente del cap\'itulo~\ref{cap5}, \S\ref{sec5.3}, teorema~\ref{teo5.3.1} que s\'olo hay {\em un n\'umero finito} de
ideales maximales de $A'$ que son ramificados sobre $A$. Nosotros caracterizaremos un ideal $\diff_{K'/K}$
de $A'$, el {\em ``diferente''} de $K'$ sobre $K$, tal que esos ideales son exactamente aquellos que contienen
...
\Tr_{K'/K}(xA') \subset A
\end{gather}
es un ideal fraccionario $\mathfrak C$ de $A'$, que lo llamamos el {\em codiferente} de $K'$ sobre $K$.
Por definici\'on el {\em diferente} $\diff_{K'/K}$ es el ideal inverso $\mathfrak C^{-1}$. Es un ideal
{\em entero} no nulo de $A'$. Se demuestra que est\'a {\em generado por los $F'(x)$,} donde $x$ recorre $A'$ y
